...
public class UartCp210x extends UartController {
	...
	protected int DEFAULT_BAUDRATE = 19200;

	public UartCp210x(Context context, UsbDeviceConnection mConnection) {
	...
	private int cp210xUsbEnable() {
		if (mConnection == null)
			return -1;
		return mConnection.controlTransfer(REQTYPE_HOST_TO_INTERFACE, CP210X_IFC_ENABLE, UART_ENABLE, 0, null, 0, 100);
	}
	...
	private int cp210xUsbDisable() {
		if (mConnection == null)
			return -1;
		return mConnection.controlTransfer(REQTYPE_HOST_TO_INTERFACE, CP210X_IFC_ENABLE, UART_DISABLE, 0, null, 0, 100);
	}
	 ...
	 *            gotten buffer
	 * @param size
	 *            size of getting buffer
	 * @return positive value : successful, negative value : fail
	 */
	private int cp210xGetConfig(int request, byte[] buf, int size) {
		if (mConnection == null)
			return -1;
		int ret = mConnection.controlTransfer(REQTYPE_INTERFACE_TO_HOST, request, 0x0000, 0, buf, size, 100);
		return ret;
	}
	...
	private int cp210xSetConfig(int request, byte[] buf, int size) {
		if (mConnection == null)
			return -1;
		int ret = mConnection.controlTransfer(REQTYPE_HOST_TO_INTERFACE, request, 0x0000, 0, buf, size, 100);
		return ret;
	}
	...
	public int getBaudrate() {
		...
		this.mConnection.controlTransfer(193, 29, 0, 0, arrayOfByte, arrayOfByte.length, 0);
		ByteBuffer localByteBuffer = ByteBuffer.wrap(arrayOfByte);
		localByteBuffer.order(ByteOrder.LITTLE_ENDIAN);
		mUartConfig.baudrate = localByteBuffer.asIntBuffer().get();
		return mUartConfig.baudrate;
	}
	...
	private void intToLittleEndianBytes(int in, byte[] out) {
		...
		// localIntBuffer.put(in);
		// out = localByteBuffer.array();
	}
